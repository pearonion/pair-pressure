...
- Include independent Slate/viewport overlays, charge meters, prompts, sounds, and cleanup paths in HUD replacement audits. A replacement is incomplete if any user-visible feedback is routed only into a disabled or asset-missing presenter.
- An opt-in or dormant framework must be dormant at component startup: no missing-asset loads, polling timers, delegate bindings, or per-actor presentation work while disabled. Audit `BeginPlay`, constructors, timers, and config loads separately from widget creation.

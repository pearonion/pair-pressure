...
- Pair Pressure uses `JumpMaxHoldTime = 0`, so every jump is a fixed-height impulse. Its release handler intentionally does not call `StopJumping`; clearing a quick tap before CharacterMovement consumes it can skip the jump entirely.
- Ragdoll expiry only makes recovery eligible: the physical-state component keeps movement locked until its hips are low-velocity over a traced ground surface, then snaps the restored capsule to that floor before enabling walking. Spinner grazes are floored at severity 30 (not 60), launch laterally using the dummy's charged-throw profile: 790 uu/s (half throw) at low speed through 1580 uu/s (full throw) at severity 100+, retain the existing 0.85–2.25 seconds of ragdoll time, and are ignored completely after the first impact while already ragdolled. The ragdoll tick clamps live hips angular velocity to 120 deg/s, preventing residual constraint energy from turning the penguin into a pinwheel.
- `AVNHShopperCharacter` enforces `JumpMaxHoldTime = 0` and the baseline `JumpZVelocity = 650` both on its CDO and in `BeginPlay`, so stale inherited Blueprint defaults cannot produce variable jump heights. All physical-state ragdoll sources are duration-bound: debug ragdoll is 1.25s; ordinary requests are 0.85–1.6s by daze; obstacle/spinner impacts and dummy throws scale from about 1.0s to a hard maximum of 3.0s. While ragdolled, additional impacts are ignored so a spinner cannot repeatedly re-launch/re-time the body. Spinner propulsion uses the same half-to-full 790–1580 uu/s profile as a charged dummy throw. Expiry restores the locomotion AnimBP directly, with no physical-state get-up animation. The compact hips/chest asset uses 4.0/4.5 angular damping to limit residual spinning without making gravity look weak. Dive recovery starts at 0.60s and unlocks at 1.02s after landing.
...
- `/Game/Maps/Lobby` contains a `Grab Test` setup: `/Game/PairPressure/Debug/BP_PP_GrabDummyPenguin` at `(205,-55,92)`, `/Game/PairPressure/Debug/BP_PP_GrabTestItem` as an 8 kg replicated soft-carry sphere at `(165,55,66)`, and an intentionally over-mass bowling ball labelled `PP_GrabTest_HeavyBowlingBall_Reject` for rejection comparison. The closer positions keep the dummy and light pickup inside their target-specific ranges from the spawn grab origin. Pair Pressure grab and dive input are enabled in `Lobby` as well as `PP_` maps. `IA_Grab` remains the primary Enhanced Input action; the configured `PP_Grab` legacy action is bound to the same guarded press/release handlers as a Lobby Game-and-UI fallback. Do not map `PP_Grab` to Assist. A successful SPACE jump arms exactly one CTRL dive while falling; landing clears the arm, so walking off a ledge cannot initiate a dive.
- The Pair Pressure follow camera uses a centered elevated party-platformer framing (620-unit boom, 112-unit raised target, -28-degree baseline pitch, 85-degree FOV) to keep the mascot, ragdoll target, and held object visible. Locomotion is camera-relative but never writes movement-facing yaw back into control rotation: the character movement component orients the mascot toward the requested world direction, so S turns and runs screen-down without rotating/oscillating the camera. Held physics-handle items remain movable with the player. Q now charges on press and throws on release, with a local bottom-center progress bar; tap power is 55% of the 1050 uu/s baseline and full charge is 215%, while grabbed players receive 80% of the same charged curve. A thrown player transitions into the normal timed physical-state ragdoll and receives velocity on all simulated bodies; a normal LMB release instead plays the mascot table's `GetUpFront` animation in single-node mode and restores its locomotion AnimBP/idle afterward. A held LMB stays in `Reaching` and retries the server target search at 12.5 Hz until release or successful acquisition. Detection combines the forward chest sphere sweep with a 145-unit lower-body proximity overlap, allowing small gameplay items directly beside or beneath the mascot; close items relax only the vertical/centering angle and still reject targets substantially behind the player. Items and grabbed ragdolls now share a carry target derived from the midpoint of `hand_L` and `hand_R` (with case/fallback handling and a chest-height floor), so the sphere and a grabbed penguin use the same in-front-of-hands handle path. Pair Pressure/Lobby penguins use `/Game/PairPressure/Characters/Penguin/PHYS_Penguin_GrabRagdoll`, a separate 19-body/18-constraint Physics Asset; its corrected body mass overrides total 45.5 kg, all bodies use 2.5 linear/5.0 angular damping, and all self-collision pairs are disabled while world collision remains active. The inversely-scaled import roots `Null` and `petArmat` are kinematic and runtime ragdoll activation uses physics blend weight so those roots are never forcibly simulated; forcing every body to simulate launched the mesh roughly 54,000 units away and caused the immediate distance release/recovery. A successful standing-player grab clears initial ragdoll velocities, disables the victim's blocking capsule/movement, and soft-holds the mascot-specific `chest` bone (with humanoid spine/hips fallbacks) at that shared hand carry target through the grabber's Physics Handle using a stable 1800 stiffness/360 damping profile and 400000 minimum break force; the grabber retains movement and jumping. Do not synchronize the grabbed Character actor/capsule to the physics mesh by teleporting both every Tick while the Physics Handle is active: that double-transform fights the handle and can launch the ragdoll. Recovery/throw release relocates the actor from the ragdoll once instead. Internal constraint failures restore without playing `GetUpFront`; that recovery animation is reserved for an explicit normal LMB release. Player and item constraints receive startup grace, immediate front targets, interpolation, wider distance tolerance, and explicit release-reason logs. The victim camera actor follows the ragdoll hips, and SPACE only recovers/launches the victim when their current movement input points sideways or away, then applies recoil/immunity. `OpponentEscapeSeconds=0` disables automatic timed release by default; a positive profile/component value can restore the earlier safety timeout. A reciprocal player-grab attempt releases both sides and gives both a short same-opponent immunity instead of entering `MutualGrab`. The Lobby light test sphere sets `Pawn -> Ignore` at BeginPlay so the player capsule cannot punt it before LMB acquisition; runtime logs confirmed it is selected as an 8 kg gameplay item.
- Follow-up ragdoll stabilization frees the `hips -> petArmat` constraint in `/Game/PairPressure/Characters/Penguin/PHYS_Penguin_GrabRagdoll` and runtime code simulates only the hips/pelvis body chain, never either scaled import root. The shared hand carry target is lowered 28 units from the hand midpoint/chest-height floor so held items sit visually between the arms. `/Game/FunObstacleCourseExp/Blueprints/BP_Spinner_V2` is tagged `PP_HeavyObstacle` and `PP_SpinnerObstacle` and enables rigid-body hit notification before starting its existing timelines. Lobby impact sensing now recognizes it server-side, forces a speed-scaled propelled ragdoll, waits for 1.5 continuous seconds of low-velocity ground contact, then plays `GetUpFront` and restores the locomotion AnimBP.
- Penguin player pickup now uses the grabber's Physics Handle against the target's simulated `chest` body and updates only the handle target; it must not teleport the Character actor/capsule every Tick while the ragdoll mesh is active. The shared Penguin Physics Asset keeps the scaled `Null`/`petArmat` import roots bodyless and explicitly enables world collision on all 15 valid `hips`-rooted bodies. Empty `Reaching` does not enable two-hand IK because the compact wing chain transfers that offset into a visible torso roll. Air dive presentation starts immediately while falling, holds the final dive pose on landing, waits one full grounded second, then plays the recovery clip.
- Ledge-grip candidate detection searches the authored `PP.Grab.Ledge`/`PP.Grab.Handle` volume separately from ordinary item sweeps and permits a 250-unit, broad-front capture; normal target selection remains strict. Release presentation now carries an explicit ledge-climb flag and resets the previous grab profile, so a past ledge release cannot contaminate later item/reach release animation selection. Penguin carry disables torso-sharing grab IK entirely; player carry uses a rotation-constrained Physics Handle at the simulated chest, preserving its initial orientation relative to the carrier while the handle follows the overhead target. Dive accepts any armed falling state, snaps instantly into a looping dive clip, and after landing holds its final pose for 0.75 seconds before recovery.
- Pair Pressure penguins use `/Game/CuteChubbyPenguin/Penguin/Meshes/PHYS_Penguin_UE_PhysicsAsset`, assigned to `/Game/CuteChubbyPenguin/Penguin/Meshes/SK_Penguin_UE` and compatible with its 19-bone `SKEL_Penguin_UE_Skeleton`. The asset has 15 shaped bodies and 14 parent constraints rooted at `hips`: chest, neck/head, both wing/hand chains, all three tail bones, and both foot/toe chains. `Null` and `petArmat` intentionally have no bodies because their reciprocal extreme import scale previously caused explosive launches; `belly_L/R` remain torso deformation followers. All simulated body pairs ignore self-collision while retaining world collision. Runtime ragdoll/carry enables Query-and-Physics world blocking plus CCD, simulates all bodies below `hips`, ignores Pawn/Camera, and the local spring-arm origin follows the simulated chest while the capsule is disabled. Spinner propulsion is capped to a 500-1450 uu/s severity curve. The shared hand carry target is lowered 45 units from the hand midpoint/chest-height floor.
...
- Pair Pressure's `/Game/PairPressure/Debug/BP_PP_GrabDummyPenguin` inherits `AVNHShopperCharacter` and has no custom grab graph/components. It must use the dedicated non-physics attachment path in `UPPGrabberComponent` while carried; do not route it through the real-player incoming-ragdoll/physics-handle path. Real player grabs remain on that path so their directional jump-away escape keeps working. Spinner knockdowns should set propulsion velocity only on the resolved ragdoll root body, never all constrained bodies.
...
- The live Penguin mesh uses `/Game/CuteChubbyPenguin/Penguin/Meshes/PHYS_Penguin_UE_PhysicsAsset`, assigned by `AVNHShopperCharacter` in Pair Pressure maps. Its stable compact-ragdoll setup is two simulated, shaped bodies: `hips` and `chest`; the hands, toes, and head remain visible as animation-driven mesh bones, not physics bodies. Chest uses a direct, tightly limited constraint to make the core fall as one stable body, and its only retained internal collision pair is disabled. Hips/chest use low linear damping (0.22/0.28) to avoid a low-gravity read and moderate angular damping (2.4/2.8) for a smooth fall. During ragdoll the capsule is disabled and the root `hips` body receives knockback/charged-throw velocity; do not use `LaunchCharacter` for the dummy's ragdoll throw. Spinner recovery requires the standard two continuous seconds of grounded rest.
